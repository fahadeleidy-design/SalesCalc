/*
  ...
  ## Overview
  SECURITY DEFINER alone doesn't bypass RLS when policies use auth.uid().
  Need to explicitly set row_security = off in the function.
  
  ## Changes
  - Add SET row_security = off to bypass RLS within function
  - Function still validates permissions manually before operations
  
  ## Security
  - Manual permission checks ensure proper access control
  - Only authorized users can convert leads
*/
...
BEGIN
  ...
  -- Get current profile ID and role (bypasses RLS due to SET row_security = off)
  ...
  -- Get lead data (bypasses RLS, we check permissions manually)
  SELECT 
    assigned_to,
