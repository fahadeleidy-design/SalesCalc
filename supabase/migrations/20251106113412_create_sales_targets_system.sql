/*
  ...
  1. New Tables
    - `sales_targets`
      ...
      - `period_type` (text, monthly/quarterly/half_yearly/yearly)
      - `period_start` (date)
      - `period_end` (date)
      - `target_amount` (decimal)
      - `status` (text, pending_ceo/approved/rejected)
    ...
    - `team_targets`
      - Similar structure for Sales Manager's team targets
  
  2. Security
    - Enable RLS on both tables
    - Sales reps can view their own targets
    - Managers can view targets they set
    - CEO can view all targets
    - Managers can create and update pending targets
    - CEO can approve/reject targets
  
  3. Functions
    - `calculate_period_dates()` - Helper to calculate period end dates
*/
...
CREATE POLICY "Sales reps can view their own targets"
  ...
  TO authenticated
  USING (
    sales_rep_id IN (SELECT id FROM profiles WHERE user_id = auth.uid())
  );
...
CREATE POLICY "Managers can view targets they set"
  ...
  TO authenticated
  USING (
    manager_id IN (SELECT id FROM profiles WHERE user_id = auth.uid())
  );
...
CREATE POLICY "Managers can create targets"
  ...
  TO authenticated
  WITH CHECK (
    manager_id IN (SELECT id FROM profiles WHERE user_id = auth.uid()) AND
    EXISTS (
      SELECT 1 FROM profiles
      WHERE profiles.user_id = auth.uid()
