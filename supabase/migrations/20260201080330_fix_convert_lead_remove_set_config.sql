/*
  # Fix Convert Lead Function - Remove set_config Calls

  ## Overview
  The previous implementation used set_config('role', 'service_role', true) which doesn't work
  for non-superusers. This fix removes those calls and relies solely on SECURITY DEFINER
  which automatically bypasses RLS when running as the function owner.

  ## Changes
  - Remove all set_config('role', ...) calls
  - Keep SECURITY DEFINER which provides proper RLS bypass
  - Simplified error handling without role reset attempts

  ## Security
  - SECURITY DEFINER runs the function as the owner (postgres), bypassing RLS
  - Manual permission checks are still performed to validate user access
  - auth.uid() still returns the actual caller for permission validation
*/
