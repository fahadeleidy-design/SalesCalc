/*
  # Add Convert Lead to Opportunity Function

  ## Purpose
  Creates a database function to atomically convert a lead to an opportunity
  while automatically creating a customer record.

  ## Features
  1. Creates a customer from lead data
  2. Creates an opportunity linked to the new customer and lead
  3. Updates lead with converted status and customer reference
  4. All operations in a single transaction

  ## Function
  - `convert_lead_to_opportunity(lead_id UUID)` - Returns the new opportunity ID
*/
